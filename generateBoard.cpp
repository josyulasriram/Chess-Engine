...
vector<int> Board::getWPMoves() {
  //Right capture
  vector<int> m;
  unsigned long long moves = (wP >> 7) & (blackPieces) & (~files[0]);
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
  ...
  //Left capture
  moves = (wP >> 9) & (blackPieces) & (~files[7]);
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
  ...
  //move two up
  moves = (((wP >> 8) & empty) >> 8) & empty & ranks[4];
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
...
vector<int> Board::getBPMoves(){
  //Right capture
  vector<int> m;
  unsigned long long moves = (bP << 7) & (whitePieces) & (~files[7]);
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
  ...
  //Left capture
  moves = (bP << 9) & (whitePieces) & (~files[0]);
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
  ...
  //move two up
  moves = (((bP << 8) & empty) << 8) & empty & ranks[3];
  for(int i = 0; i < 64; i++){
    if((moves >> i) & 1){
      m.push_back(i);
    }
